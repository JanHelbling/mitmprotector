...
class mitmprotector(object):
    ...
    def __arptable_firewall__(self):
        ...
        print("Interface: {0}\nRouter-IP: {1}".format(self.interface, self.routerip))
        self.data = popen(
            "arping -c 1 -i {0} {1} 2>&1 | grep {1}".format(
                self.interface, self.routerip
            ),
            "r",
        ).read()
        try:
            ...
            print("Router-MAC: {}".format(self.mac))
        popen(
            "arptables --zero && arptables -P INPUT DROP && arptables -P OUTPUT DROP && arptables -A INPUT -s {0} --source-mac {1} -j ACCEPT && arptables -A OUTPUT -d {0} --destination-mac {1} -j ACCEPT && arp -s {0} {1}".format(
                self.routerip, self.mac
            ),
            "r",
        )
        print(
            "arptables --list:\n{}".format(
                popen("arptables --list", "r").read().rstrip("\n")
            )
        )
        arptables_used = True
    ...
    def __run__(self):
        ...
        while True:
            ...
            if self.attacker != ():
                ...
                print(
                    "Exexute predefined command: '{}'!".format(
                        self.cmd.format(self.attacker[0], self.attacker[1])
                    )
                )
                ...
                if not self.pid:
                    ...
                    if self.putinterfacedown:
                        print(
                            "Shut down the networkinterface {}!".format(self.interface)
                        )
                        critical(
                            "Shut down the networkinterface {}!".format(self.interface)
                        )
                ...
                if self.putinterfacedown:
                    ...
                    exit(0)
            print(
                "[{0}] Sleeping {1} seconds until the next check.".format(
                    self.counter, self.scan_timeout
                )
            )
    ...
    def __arp__(self):
        ...
        for line in self.lines:
            ...
            try:
                ...
                print(
                    'IndexError: Failed to regex a line from "{0}" => IP & MAC'.format(
                        self.arp_command
                    )
                )
            ...
            for _device in self.devices:
                if device[0] != _device[0] and device[0] != self.routerip:
                    if device[1] == _device[1]:
                        ...
                        critical(
                            "MITM ATTACK DETECTED!!! IP: {0} , MAC: {1}".format(
                                self.attacker[0], self.attacker[1]
                            )
                        )
                        print(
                            "MITM ATTACK DETECTED!!! IP: {0} , MAC: {1}".format(
    ...
    def __getrouterip__(self):
        if path.exists("/proc/net/route"):
            try:
                ...
                critical(
                    'Error: Couldn\'t open "/proc/net/route": {}.'.format(e.strerror)
                )
                info("Trying 3 alternate methods to get the RouterIP.")
            ...
            info(
                'File "/proc/net/route" doesn\'t exists! Trying 3 alternate methods to get the RouterIP.'
            )
            print(
                'File "/proc/net/route" doesn\'t exists! Trying 3 alternate methods to get the RouterIP.'
            )
        try:
            ...
            return findall(
                "\d+\.\d+\.\d+\.\d+", popen("route -n").read().split("\n")[2]
            )[1]
        except IndexError:
            try:
                info("=> Method 3: ifconfig {} 2>&1 | grep inet".format(self.interface))
                print(
                    "=> Method 3: ifconfig {} 2>&1 | grep inet".format(self.interface)
                )
                return (
                    findall(
                        "\d+\.\d+\.\d+\.\d+",
                        popen(
                            "ifconfig {} 2>&1 | grep inet\ ".format(self.interface)
                        ).read(),
                    )[0].rstrip("1234567890")
                    + "1"
                )
                ...
                info(
                    "=> Method 4: Guessing (ping -c 1 -W 1 -I {} 192.168.0.1/192.168.1.1/192.168.8.1)".format(
                        self.interface
                    )
                )
                print(
                    "=> Method 4: Guessing (ping -c 1 -W 1 -I {} 192.168.0.1/192.168.1.1/192.168.2.1)".format(
                        self.interface
                    )
                )
                if (
                    popen(
                        'ping -W 1 -I {} 192.168.0.1 -c 1 2>/dev/null | grep -E "1 received"'.format(
                            self.interface
                        )
                    ).read()
                    != ""
                    ...
                    popen(
                        'ping -W 1 -I {} 192.168.1.1 -c 1 2>/dev/null | grep -E "1 received"'.format(
                            self.interface
                        )
                    ).read()
                    != ""
                    ...
                    popen(
                        'ping -W 1 -I {} 192.168.2.1 -c 1 2>/dev/null | grep -E "1 received"'.format(
                            self.interface
                        )
                    ).read()
                    != ""
...
class script_manager(object):
    ...
    def remove_scripts(self):
        if path.exists("/etc/network/if-post-down.d/mitmprotector") and path.exists(
            "/etc/network/if-up.d/mitmprotector"
        ):
            ...
            try:
                ...
                exit(1)
        if path.exists("/etc/wicd/scripts/predisconnect/mitmprotector") and path.exists(
            "/etc/wicd/scripts/postconnect/mitmprotector"
        ):
    ...
    def add_scripts(self):
        if path.exists("/etc/network/if-post-down.d/") and path.exists(
            "/etc/network/if-up.d/"
        ):
            if path.exists("/etc/network/if-post-down.d/mitmprotector") and path.exists(
                "/etc/network/if-up.d/mitmprotector"
                ...
                try:
                    mitmprotector_down = open(
                        "/etc/network/if-post-down.d/mitmprotector", "w"
                    )
                    mitmprotector_up = open("/etc/network/if-up.d/mitmprotector", "w")
                    mitmprotector_down.write(
                    ...
                    print(
                        "[NetworkManager] Failed to create {}: {}.".format(
                ...
                try:
                    ...
                    print(
                        "[NetworkManager] Failed to chmod->755 {}: {}.".format(
                            e.filename, e.strerror
                        )
                    )
                    ...
                    exit(1)
                print(
                    "[NetworkManager] Created /etc/network/if-post-down.d/mitmprotector and /etc/network/if-up.d/mitmprotector => 755"
                )
        ...
        if path.exists("/etc/wicd/scripts/postconnect/") and path.exists(
            "/etc/wicd/scripts/predisconnect/"
        ):
            if path.exists(
                "/etc/wicd/scripts/predisconnect/mitmprotector"
            ) and path.exists("/etc/wicd/scripts/postconnect/mitmprotector"):
                print("[WICD] Scripts already installed!")
            else:
                print("[WICD] Found! Installing scripts.")
                try:
                    mitmprotector_down = open(
                        "/etc/wicd/scripts/predisconnect/mitmprotector", "w"
                    )
                    mitmprotector_up = open(
                        "/etc/wicd/scripts/postconnect/mitmprotector", "w"
                    )
                    mitmprotector_down.write(
                        "#!/bin/bash\npkill -TERM -F /var/run/mitmprotector.pid"
                    )
                    ...
                    print(
                        "[WICD] Failed to create {}: {}.".format(e.filename, e.strerror)
                    )
                    exit(1)
                try:
                    ...
                    print(
                        "[WICD] Failed to chmod->755 {}: {}.".format(
                            e.filename, e.strerror
                        )
                    )
                    print("    You must manual chmod 755 these files:")
                    ...
                    exit(1)
                print(
                    "[WICD] Created /etc/wicd/scripts/predisconnect/mitmprotector and /etc/wicd/scripts/postconnect/mitmprotector => 755"
                )
...
if __name__ == "__main__":
    ...
    parser.add_option(
        ...
        action="store_true",
        default=False,
        help="Run mitmprotector as a daemon.",
    )
    parser.add_option(
        "-f",
        "--foreground",
        ...
        action="store_true",
        default=True,
        help="Run mitmprotector in foreground.",
    )
    parser.add_option(
        "-k",
        "--kill",
        ...
        action="store_true",
        default=False,
        help="Kill mitmprotector with SIGTERM!",
    )
    parser.add_option(
        "-n",
        "--nm-aoc",
        ...
        action="store_true",
        default=False,
        help="Enable  NetworkManager/WICD -autostartscripts",
    )
    parser.add_option(
        "-r",
        "--rm-aoc",
        ...
        action="store_true",
        default=False,
        help="Disable NetworkManager/WICD -autostartscripts",
    )
